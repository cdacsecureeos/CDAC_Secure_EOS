...
/* added part*/
static int caam_get_random(unsigned char *key, size_t key_len)
{
...
struct trusted_key_ops trusted_key_caam_ops = {
	...
	.exit = trusted_caam_exit,
	 .get_random = caam_get_random,  //  ADD THIS LINE
};
// Use late_initcall so it runs after subsystems are ready
/*late_initcall(trusted_caam_init);
module_exit(trusted_caam_exit);

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Trusted Keys backed by CAAM");*/
